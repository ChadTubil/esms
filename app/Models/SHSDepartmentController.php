...
class SHSDepartmentController extends BaseController
{
    ...
    public function admissionProcess($id=null){
        ...
        ->where('students_shs.studid', $id)
        ->findAll();
        
        // $studentsshsdata2 = $this->shsStudentsModel
        // ->select('students_shs.*, permanentrecord.*')
        // ->join('permanentrecord', 'permanentrecord.studid = students_shs.studid', 'left')
        // ->where('students_shs.studid', $id)->findAll();
        
        // $data['studentsshsdata'] = array_merge($studentsshsdata1, $studentsshsdata2);
        ...
        if($this->request->is('post')){
            ...
            // ];
            // $this->shsStudentsModel->where('studid', $studid)->update($studid, $STUDdata);
    ...
    public function admissionProcessGenerate($id=null) {
        $year = date('y');
        $studentNumber = $year . 'S' . str_pad($id, 4, '0', STR_PAD_LEFT);
        $data = [
    ...
    public function advisingSubmitAccount($id=null) {
        ...
        $this->enrollmentHistorySHSModel->where('studid', $STUDENTID)->set(['status' => 'Assessed'])->update();
    ...
    public function assessmentPrint($id=null) {
        ...
        ->select('assessment_shs.*, curriculum_shs.*, sections_shs.*, students_shs.*,shsstudentschoolrecord.*')
        ->join('shsstudentschoolrecord', 'shsstudentschoolrecord.studid = assessment_shs.studid', 'left')
        ->join('curriculum_shs', 'curriculum_shs.currid = assessment_shs.curriculum')
        ->join('sections_shs', 'sections_shs.secid = assessment_shs.section')
        ->join('students_shs', 'students_shs.studid = assessment_shs.studid')
        ->where('assessment_shs.studid', $STUDID)
        ...
        $html = '
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>LRN: <strong>'. strtoupper($LRN) .'</strong></td>
                </tr>
                <tr>
                    <td>VOUCHERS: <strong>'. strtoupper($VOUCHERS) .'</strong></td>
                </tr>
            </table><br><br>
