                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            ...
                            <table id="datatable" class="table table-striped" data-toggle="data-table">
                                ...
                                <tbody>
                                    <?php foreach($enrollmenthistorycoldata as $enrollmenthistorycold):?>
                                        <tr>
                                            <td style="text-align: left;">
                                                <?php
                                                    if($enrollmenthistorycold['studentno'] == '') {
                                                        echo "NEW STUDENT";
                                                    } else {
                                                        echo $enrollmenthistorycold['studentno'];
                                                    } 
                                                ?>
                                            </td>
                                            <td style="text-transform: uppercase; text-align: left;"><?= $enrollmenthistorycold['studfullname']; ?></td>
                                            <td style="text-align: center;">
                                                <?php 
                                                    if($enrollmenthistorycold['sy'] == '') {
                                                        echo "-";
                                                    } else {
                                                        echo $enrollmenthistorycold['sy'];
                                                    }
                                                ?>
                                            </td>
                                            <td style="text-align: center;">
                                                <?php 
                                                    if($enrollmenthistorycold['course'] == '') {
                                                        echo "-";
                                                    } else {
                                                        echo $enrollmenthistorycold['code'];
                                                    }
                                                ?>
                                            </td>
                                            <td style="text-align: center;">
                                                <?php 
                                                    if($enrollmenthistorycold['level'] == '') {
                                                        echo "-";
                                                    } else {
                                                        echo $enrollmenthistorycold['level'];
                                                    }
                                                ?>
                                            </td>
                                            <td style="text-align: center;"><?php echo $formattedDate = date('F j, Y', strtotime($enrollmenthistorycold['date'])); ?>
                                            </td>
                                            <td style="text-align: center;">
                                                <a class="btn btn-sm btn-icon btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="ADVISE"
                                                    onclick="window.location.href='<?= base_url(); ?>col-advising/process/<?= $enrollmenthistorycold['studid']; ?>';">
                                                ...
                                                <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="CANCEL"
                                                    onclick="window.location.href='<?= base_url(); ?>col-admission/process-cancel/<?= $enrollmenthistorycold['ehid']; ?>'">
                                                    <span class="btn-inner">
                                                        <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
