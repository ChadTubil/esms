...
<?= $this->section("page_p"); ?>
    <?= $page_p; ?>
<?= $this->endSection(); ?>
<?php foreach($assessmentdata as $ASSD){
        $ASSID = $ASSD['assid'];
    } 
?>
            ...
            <div class="col-lg-7 col-sm-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-sm-12">
                                <label for="">CHANGE SECTION</label>
                                <?= form_open('col-change-enrollment/process-assessment-changeschedule-updatesection/'.$ASSID); ?>
                                    <div class="form-group" style="display: flex; gap: 10px; justify-content: center;">
                                        <select name="section" class="form-select" required>
                                            ...
                                            <?php foreach($changesectiondata as $changesectiondata): ?>
                                                <option value="<?= $changesectiondata['secid']; ?>"><?= $changesectiondata['section']; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                        <button type="submit" class="btn btn-m btn-icon btn-primary" style="width: 20%;">UPDATE</button>
                                    </div>
                                <?= form_close(); ?>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <?php foreach($studentsubjectsdata as $ssd): ?>
                                            <tr>
                                                ...
                                                <td style="text-align: center; ">
                                                    <?php if($ssd['section'] == "0"): ?>
                                                        <?= form_open('col-change-enrollment/process-assessment-changeschedule-updatesubjectsched/'.$ssd['ssid'].'/'.$ASSID); ?>
                                                            <div class="form-group" style="display: flex; gap: 10px; justify-content: center;">
                                                                <select class="form-select" name="subjectsection">
                                                                    <option value="0" selected>No schedule assigned</option>
                                                                    <?php foreach($colsectiondata as $colsectiond): ?>
                                                                        <option value="<?= $colsectiond['secid']; ?>">
                                                                            <?= $colsectiond['section']; ?> - 
                                                                            (<?= $colsectiond['student_count'] ?? 0; ?> Students)
                                                                        </option>
                                                                    <?php endforeach; ?>
                                                                </select>
                                                    ...
                                                    <?php else: ?>
                                                        <?= form_open('col-change-enrollment/process-assessment-changeschedule-updatesubjectsched/'.$ssd['ssid'].'/'.$ASSID); ?>
                                                            <div class="form-group" style="display: flex; gap: 10px; justify-content: center;">
                                                                <select class="form-select" name="subjectsection">
                                                                    <?php foreach($colsectiondata as $colsectiond): ?>
                                                                        <?php if($colsectiond['secid'] == $ssd['section']): ?>
                                                                            <option value="<?= $colsectiond['secid']; ?>" selected>
                                                                                <?= $colsectiond['section']; ?> - 
                                                                                (<?= $colsectiond['student_count'] ?? 0; ?> Students)
                                                                            </option>
                                                                        <?php else: ?>
