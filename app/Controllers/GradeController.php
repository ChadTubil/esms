...
class GradeController extends BaseController
{
        ...
        if($this->request->is('post')) {
            ...
            if($searchStudent == '') {
                ...
                if(empty($oldStudents)) {
                    $data['resultStudent'] = $colStudents;
                }else {
                    $data['resultStudent'] = $oldStudents;
                }
                return view('gradeviewresult', $data);
            }
        }
    ...
    public function gradeView($id=null) {
        ...
        if(empty($oldStudents)){
            $data['accountinfo'] = $colStudents;
        }else {
            $data['accountinfo'] = $oldStudents;
        }
    ...
    public function gradeViewResultNew($id=null) {
        ...
        foreach($colStudents as $account){
            $studid= $account['studid'];
        }
        $syid = session()->get('selected_sy');
        $semid = session()->get('selected_sem');
        $data['selectedsy'] = $this->syModel->where('syname', $syid)->findAll();
        ...
        // print_r($importedGradeCondition);
    ...
    public function gradeViewUpdateNew($ssid=null) {
        if($this->request->is('post')) {
            $studid = $this->request->getVar('studnotext');
            $studentdata = $this->colstudentsModel->where('studid', $studid)->findAll();
            foreach($studentdata as $stud){
                $studno = $stud['studentno'];
            }
            $data = [
                'prelim' => $this->request->getVar('prelimtext'),
                'midterm' => $this->request->getVar('midtermtext'),
            ...
            $this->subjectSubjectsModel->where('ssid', $ssid)->update($ssid, $data);
            // UPDATE SEMESTRAL
            $db = \Config\Database::connect();
            ...
            session()->setTempdata('updatesuccess', 'Update is Successful!', 2);
            return redirect()->to(base_url()."gradesview/resultnew/".$studno);
        }
    }
    public function gradesDownload() {
        ...
        if($this->request->is('post')){
            ...
            if($sy == '2022-2023' || $sy == '2023-2024' || $sy == '2024-2025'){
                ...
                $syinfo = $db->query("SELECT * FROM sy WHERE syname = '$sy'");
                $syresult = $syinfo->getRow(0);
                $GETSY = $syresult->syname;

                $seminfo = $db->query("SELECT * FROM semester WHERE semester = '$sem'");
                ...
                if($sem == '1st Semester' || $sem == '2nd Semester'){
                    ...
                    $studinfo = $db->query("SELECT * FROM students_col WHERE studentno = '$studentnum'");
                    $studresult = $studinfo->getRow(0);
                    $STUDID = $studresult->studid;
                    ...
                    $studexp = $this->colassessmentModel
                    ->select('assessment_col.*, courses.*')
                    ->join('courses', 'courses.courid = assessment_col.course')
                    ->where(['assessment_col.studid' => $STUDID, 'assessment_col.sy' => $GETSY, 'assessment_col.sem' => $GETSEM])->findAll();
                    foreach($studexp as $row){
                        $LEVEL = $row['level'];
                        $COURSE = $row['name'];
                    }

                    $totalunits = $this->subjectSubjectsModel
                    ->select('SUM(subjects.units) as TOTALUNITS')
                    ->join('currdata', 'currdata.cdid = student_subjects.cdid')
                    ->join('subjects', 'subjects.subid = currdata.subid')
                    ->WHERE(['student_subjects.studid' => $STUDID, 'student_subjects.sy' => $GETSY, 'student_subjects.sem' => $GETSEM])
                    ->findAll();
                    foreach($totalunits as $row){
                        $SUMUNITS = $row['TOTALUNITS'];
                    }

                    $semestralxunits = $this->subjectSubjectsModel
                    ->select('SUM(student_subjects.semestral * subjects.units) as COMPUTED')
                    ->join('currdata', 'currdata.cdid = student_subjects.cdid')
                    ->join('subjects', 'subjects.subid = currdata.subid')
                    ->WHERE(['student_subjects.studid' => $STUDID, 'student_subjects.sy' => $GETSY, 'student_subjects.sem' => $GETSEM])
                    ->findAll();
                    foreach($semestralxunits as $row){
                    ...
                    $html = '
                        ...
                        <table>
                            ...
                            <tr>
                                <td><strong>Course: </strong>'.strtoupper($COURSE).'</td>
                                <td><strong>Semester: </strong>'.$semresult->semester.'</td>
                            </tr>
                            <tr>
                                <td><strong>Address: </strong>'.$studresult->studstbarangay.' '.$studresult->studcity.', '.$studresult->studprovince.'</td>
                            ...
                            <tr>
                                <td></td> 
                                <td><h3 style="color: red;"><strong>GWA: '.round($COMPUTED / $SUMUNITS, 0).'</strong></h3></td>
                            </tr>
                    </table><br><br>
                    ...
                    <table border="1" style="width: 100%;">
                        ...
                        <tbody>           
                    ';
                    $igmCondition = array('student_subjects.studid' => $STUDID, 'student_subjects.sy' => $GETSY, 'student_subjects.sem' => $GETSEM);
                    $igm = $this->subjectSubjectsModel
                    ->select('student_subjects.*, subjects.subcode, subjects.subject, subjects.units')
                    ->join('currdata', 'currdata.cdid = student_subjects.cdid')
                    ->join('subjects', 'subjects.subid = currdata.subid')
                    ->where($igmCondition)->findAll();
                    ...
                    foreach($igm as $row){
                        ...
                        $html .= '<tr>
                            <td style="width: 15%;">'.$row['subcode'].' </td>
                            <td style="width: 39%;"> '.$row['subject'].' </td>
                            <td style="width: 8%; text-align: center;"> '.$FORMATED.'</td>
                            <td style="width: 7%; text-align: center;"> '.$EQUIVALENT.'</td>
                            <td style="width: 10%; text-align: center;"> '.$REMARK.'</td>
                            <td style="width: 19%; text-align: center; font-size: 9px"> '.$row['teachername'].'</td>
