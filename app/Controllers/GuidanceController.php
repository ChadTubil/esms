...
class GuidanceController extends BaseController
{
    ...
    public function __construct() {
        ...
        $this->SHSfbModel = new SHSFamilyBackgroundModel();
        $this->SHSprModel = new SHSPermanentRecordModel();
        $this->SHSsrModel = new SHSSchoolRecordModel();
        $this->SHSstudModel = new SHSStudentsModel();
    ...
    public function index()
    {
        ...
        $data['admissionrecordsshs'] = $this->SHSstudModel
        ->select('students_shs.*, permanentrecord_shs.*, shsstudentschoolrecord.*, familybackground_shs.*, additionalinfo_shs.*,')
        ->join('permanentrecord_shs', 'permanentrecord_shs.studid = students_shs.studid', 'left')
        ->join('familybackground_shs', 'familybackground_shs.studid = students_shs.studid', 'left')
        ->join('shsstudentschoolrecord', 'shsstudentschoolrecord.studid = students_shs.studid', 'left')
        ->join('additionalinfo_shs', 'additionalinfo_shs.studid = students_shs.studid', 'left')
        ->where('students_shs.studisdel', 0)
        ->findAll();
    ...
    public function printStudInfo($id=null){
        ...
        $studinfo = $db->query("SELECT * FROM students_shs WHERE studid = '$id'");
        $studresult = $studinfo->getRow(0);
        $studinfoData = $this->SHSstudModel->select('students_shs.*, permanentrecord_shs.*, shsstudentschoolrecord.*, familybackground_shs.*, additionalinfo_shs.*, clusters.*')
        ->join('clusters', 'clusters.cluid = students_shs.course', 'left')
        ...
        ->join('additional_information', 'additional_information.studid = students.studid', 'left')
        ...
        $regData = $this->studentsModel->where('studid',$STDStudId)->findAll();
        ...
        $aiData = $this->addinfoModel->where('studid',$STDStudId)->findAll();
